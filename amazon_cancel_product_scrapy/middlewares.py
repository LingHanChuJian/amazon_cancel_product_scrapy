...
from .utils import PROXY_UA_URL
...
class AmazonCancelProductScrapySpiderMiddleware(object):
...
class AmazonCancelProductScrapyDownloaderMiddleware(object):
    # Not all methods need to be defined. If a method is not defined,
    ...
    @staticmethod
    def is_add_proxy(url):
        for item in PROXY_UA_URL:
            if url.find(item) > -1:
                return True
        return False
